...
const fileContent = fs.readFileSync('./words-level2d.js', 'utf8');
const match = fileContent.match(/const LEVEL2D_BANK\s*=\s*(\[[\s\S]*\]);/);

if (!match) {
  console.error('Could not parse words-level2d.js');
  process.exit(1);
}

const wordBank = eval(match[1]);

let output = '# Gemini Verification Report: words-level2d.js\n\n';
...
fs.writeFileSync('VERIFY-GEMINI-words-level2d-GATE.md', output);
