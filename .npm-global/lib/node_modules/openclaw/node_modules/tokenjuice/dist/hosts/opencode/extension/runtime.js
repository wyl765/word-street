// src/hosts/opencode/extension/runtime.ts
...
var COMPOUND_SHELL_OPERATORS = /* @__PURE__ */ new Set([";", "\n", "|", "&&", "||"]);
var SEQUENTIAL_SHELL_OPERATORS = /* @__PURE__ */ new Set([";", "\n", "&&", "||"]);
var ENV_ASSIGNMENT_PATTERN = /^[A-Za-z_][A-Za-z0-9_]*=.*/u;
...
var SETUP_WRAPPER_COMMANDS = /* @__PURE__ */ new Set(["cd", "pwd", "set", "source", ".", "export", "unset", "trap"]);
var SHELL_COMMAND_LAUNCHERS = /* @__PURE__ */ new Set(["bash", "sh", "zsh", "fish"]);
var ENV_FLAGS_WITH_VALUES = /* @__PURE__ */ new Set(["-u", "--unset", "-C", "--chdir", "-S", "--split-string"]);
var ENV_FLAGS = /* @__PURE__ */ new Set(["-i", "--ignore-environment", "-0", "--null", "--debug"]);
...
function isLikelyShellLauncher(launcherName, launcherPath) {
  ...
  if (normalized === "dash" || normalized === "ksh" || normalized === "mksh" || normalized === "ash" || normalized === "csh" || normalized === "tcsh") {
    return true;
  }
...
function resolveEffectiveCommand(input) {
  ...
  for (const segment of segments) {
    ...
    const effectiveCommand = stripLeadingEnvAssignmentsFromCommand(trimmedSegment) ?? void 0;
...
var REPO_INVENTORY_COMMANDS = /* @__PURE__ */ new Set(["find", "fd", "fdfind", "ls"]);
var REPO_INSPECTION_ONLY_COMMANDS = /* @__PURE__ */ new Set(["tree"]);
var FIND_EXEC_ACTIONS = /* @__PURE__ */ new Set(["-exec", "-execdir", "-ok", "-okdir"]);
var FD_EXEC_OPTIONS = /* @__PURE__ */ new Set(["-x", "--exec", "-X", "--exec-batch"]);
var HEAD_FILTER = {
  flags: ["-q", "-v", "-z"],
...
var REPO_INVENTORY_PIPE_FILTERS = /* @__PURE__ */ new Map([
  ["head", HEAD_FILTER],
  ["sort", SORT_FILTER],
  ["tail", TAIL_FILTER],
  ["uniq", UNIQ_FILTER],
...
var LARGE_INSPECTION_MIN_CHARS = 4e3;
var LARGE_INSPECTION_MIN_LINES = 40;
var PACKAGE_LOCK_RE = /\bpackage-lock\.json\b/u;
...
function isLargeDocumentOutput(lines, rawChars) {
  ...
  return documentLines >= 20 && documentLines / nonEmptyLines.length >= 0.5 && codeLines / nonEmptyLines.length < 0.25;
}
function buildInspectionSummary(input, rawText) {
  const command = input.command ?? "";
  if (PACKAGE_LOCK_RE.test(command)) {
    const lines2 = buildPackageLockSummary(parseJsonValue(rawText));
    return lines2.length > 0 ? { lines: lines2, matchedReducer: "generic/package-lock-summary", compaction: createCompactionMetadata("inspection-package-lock-summary") } : null;
  }
...
// src/rules/build/go-build.json
var go_build_default = {
  id: "build/go-build",
  family: "build-go",
...
// src/rules/build/pnpm-build.json
var pnpm_build_default = {
  id: "build/pnpm-build",
  family: "build-script",
...
// src/rules/devops/terraform.json
var terraform_default = {
  id: "devops/terraform",
  family: "iac-cli",
...
// src/rules/devops/terragrunt.json
...
// src/rules/filesystem/git-ls-files.json
var git_ls_files_default = {
...
// src/rules/filesystem/rg-files.json
var rg_files_default = {
...
// src/rules/git/worktree-list.json
var worktree_list_default = {
...
// src/rules/package/composer.json
...
var BUNDLED_BUILTIN_RULES = [
  ...
  cmake_default,
  // rules/build/cmake.json
  dotnet_default,
  // rules/build/dotnet.json
  esbuild_default,
  // rules/build/esbuild.json
  go_build_default,
  // rules/build/go-build.json
  gradle_default,
  // rules/build/gradle.json
  maven_default,
  // rules/build/maven.json
  msbuild_default,
  // rules/build/msbuild.json
  pnpm_build_default,
  // rules/build/pnpm-build.json
  swift_build_default,
  ...
  pulumi_default,
  // rules/devops/pulumi.json
  terraform_default,
  // rules/devops/terraform.json
  terragrunt_default,
  // rules/devops/terragrunt.json
  fd_default,
  // rules/filesystem/fd.json
  find_default,
  // rules/filesystem/find.json
  git_ls_files_default,
  // rules/filesystem/git-ls-files.json
  ls_default,
  // rules/filesystem/ls.json
  rg_files_default,
  // rules/filesystem/rg-files.json
  fallback_default,
  ...
  worktree_list_default,
  // rules/git/worktree-list.json
  bun_install_default,
  // rules/install/bun-install.json
  npm_ci_default,
  // rules/install/npm-ci.json
  npm_install_default,
  ...
  composer_default,
  // rules/package/composer.json
  dnf_install_default,
  // rules/package/dnf-install.json
  fnm_default,
  // rules/package/fnm.json
  npm_ls_default,
  // rules/package/npm-ls.json
  yum_install_default,
  ...
  mise_default,
  // rules/task/mise.json
  node_default,
  // rules/task/node.json
  php_default,
  // rules/task/php.json
  python_default,
  // rules/task/python.json
  ruby_default,
  // rules/task/ruby.json
  uv_default,
  // rules/task/uv.json
  bun_test_default,
...
function normalizeExecutionInput(input) {
  ...
  if (shellWrapped) {
    const effectiveCommand2 = stripLeadingCdPrefix(shellWrapped);
    if (isCompoundShellCommand(effectiveCommand2)) {
      const { argv: _argv, ...restInput } = input;
      return {
        ...restInput,
        command: effectiveCommand2
      };
    }
    const argv2 = resolveEffectiveCommand({ command: effectiveCommand2 })?.argv ?? tokenizeCommand(effectiveCommand2);
    if (argv2.length === 0) {
      return {
        ...input,
        command: effectiveCommand2
...
function getDefaultArtifactDir() {
  ...
  return join2(homedir2(), ".tokenjuice", "artifacts");
}
...
function buildMetadataOnlyPath(id, storeDir) {
  ...
  return join2(resolveArtifactBaseDir(storeDir), `${id}.meta.json`);
}
...
var GITHUB_ACTIONS_EXIT_PATTERN = /^Error: Process completed with exit code (\d+)\.?$/u;
var GITHUB_ACTIONS_RUN_PATTERN = /^(?:##\[group\]|::group::)?Run\s+(.+)$/u;
var MAX_LISTED_COMMANDS = 24;
...
function dedupeAdjacent2(values) {
  const deduped = [];
...
function extractRunCommands(lines) {
  ...
  return dedupeAdjacent2(commands).filter((line) => !isShellSetupLine(line));
}
...
var LONG_SEARCH_LINE_MAX_CHARS = 420;
var LONG_CHANGED_LINE_MAX_CHARS = 260;
var GIT_DIFF_CHANGED_LINES_PER_HUNK = 8;
var GH_RUN_LOG_SIGNAL_CONTEXT_LINES = 1;
var GH_RUN_LOG_SIGNAL_PATTERN = /(?:##\[error\]|::error|(?:^|[\s|])(?:FAIL|FAILED|FAILURE)(?:[\s|:]|$)|\bAssertionError\b|\bError:\s+Process completed with exit code\b|\berror\s+TS\d+\b|\bELIFECYCLE\b|\bCommand failed\b|\bfailed with exit code\b|\bfailed in build-artifacts\b|\bERR_[A-Z0-9_]+\b)/iu;
var MAX_GH_STATUS_CHECK_ATTENTION_LINES = 12;
...
function extractGhDuration(record) {
  ...
  if (startedAt !== null && completedAt !== null && completedAt >= startedAt) {
    return formatDuration((completedAt - startedAt) / 1e3);
  }
  ...
  if (createdAt !== null && updatedAt !== null && updatedAt >= createdAt) {
    return formatDuration((updatedAt - createdAt) / 1e3);
  }
  return null;
}
...
function formatGhJsonValue(value) {
  const compactions = [];
  if (Array.isArray(value)) {
    const lines2 = value.flatMap((entry) => {
...
function rewriteSearchLines(lines) {
  ...
  const rewritten = lines.map((line) => {
    ...
    if (!match) {
      const clipped2 = clipMiddleWithHash(line, LONG_SEARCH_LINE_MAX_CHARS);
      if (clipped2.compaction) {
        compactions.push(clipped2.compaction);
      }
      return clipped2.text;
    }
...
var SMALL_OUTPUT_PASSTHROUGH_MIN_SAVED_CHARS = 120;
var SMALL_OUTPUT_PASSTHROUGH_MAX_RATIO = 0.75;
...
async function reduceExecution(input, opts = {}) {
  const rules = await loadRules(opts.cwd ? { cwd: opts.cwd } : void 0);
...
async function reduceExecutionWithRules(input, rules, opts = {}) {
  ...
  if (inspectionSummary) {
    ...
    const selectedText2 = clampTextMiddleWithMetadata(summaryText, opts.maxInlineChars ?? 1200);
    const reducedChars2 = countTextChars(selectedText2.text);
    ...
    const stats2 = {
      rawChars: measuredRawChars,
      reducedChars: reducedChars2,
      ratio: measuredRawChars === 0 ? 1 : reducedChars2 / measuredRawChars
    };
    const rawRef2 = opts.store ? await storeArtifact(
      {
        input: normalizedInput,
    ...
    return {
      inlineText: selectedText2.text,
      previewText: summaryText,
      compaction: mergeCompactionMetadata(inspectionSummary.compaction, selectedText2.compaction),
      ...trace ? { trace } : {},
      ...rawRef2 ? { rawRef: rawRef2 } : {},
      stats: stats2,
      classification: summaryClassification
  ...
  if (githubActionsFailureSummary) {
    const maxInlineChars2 = opts.maxInlineChars ?? 1200;
    const inlineText2 = clampTextMiddleWithMetadata(githubActionsFailureSummary.text, maxInlineChars2);
    const reducedChars2 = countTextChars(inlineText2.text);
    const stats2 = {
      rawChars: measuredRawChars,
      reducedChars: reducedChars2,
      ratio: measuredRawChars === 0 ? 1 : reducedChars2 / measuredRawChars
    ...
    return {
      inlineText: inlineText2.text,
      previewText: githubActionsFailureSummary.text,
      compaction: mergeCompactionMetadata(githubActionsFailureSummary.compaction, inlineText2.compaction),
      ...trace ? { trace } : {},
      ...rawRef2 ? { rawRef: rawRef2 } : {},
      stats: stats2,
      classification
...
function getOutputAwareInspectionSkipReason(policy, executionInput) {
  ...
  if (inspectionSkipReason === "file-content-inspection-command" && buildInspectionSummary(executionInput, rawText)) {
    return null;
  }
...
// src/hosts/shared/tool-result.ts
...
// src/hosts/pi/extension/utils.ts
...
function readCommand(args) {
  return isRecord2(args) && typeof args.command === "string" ? args.command : "";
}
function readCwd(args) {
  if (isRecord2(args) && typeof args.workdir === "string" && args.workdir.trim()) {
    return args.workdir;
  }
  return void 0;
}
function extractFullOutputPath(metadata) {
...
function createTokenjuiceOpenCodeExtension(_config = {}) {
  return async function tokenjuiceOpenCodeExtension(_context) {
    return {
...
[${buildCompactionNotice(outcome.result, fullOutputPath)}]`;
        ...
        if (isRecord2(output.metadata)) {
          output.metadata.tokenjuice = tokenjuiceDetails;
        } else {
          output.metadata = { tokenjuice: tokenjuiceDetails };
        }
      }
...
export {
  createTokenjuiceOpenCodeExtension
};
