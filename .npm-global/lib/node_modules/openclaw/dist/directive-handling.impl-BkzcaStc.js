...
import { b as resolveAgentDir, p as resolveSessionAgentId } from "./agent-scope-B6RIBoEj.js";
...
import { _ as modelKey, f as resolveConfiguredModelRef, h as resolveModelRefFromString } from "./model-selection-shared-BOD321LE.js";
import { n as isThinkingLevelSupported, o as resolveSupportedThinkingLevel, t as formatThinkingLevels } from "./thinking-9QU1BJ3m.js";
...
import { n as resolveAuthProfileOrder, s as isProfileInCooldown } from "./order-D7ISOGDk.js";
import { d as resolveUsableCustomProviderApiKey } from "./model-auth-CrRmREMW.js";
import { i as resolveQueueSettings, l as refreshQueuedFollowupSession } from "./queue-DzLm9htz.js";
...
import { d as renderExecTargetLabel } from "./bash-tools.exec-runtime-DooylU3X.js";
import { n as resolveExecDefaults } from "./exec-defaults-DSbiVD4i.js";
import { i as resolveModelsCommandReply } from "./commands-models-BaQtLnAd.js";
import { n as resolveSelectedAndActiveModel } from "./model-runtime-Brp9NKfs.js";
import { r as formatRemainingShort } from "./auth-health-D6-TTm35.js";
import { t as maskApiKey } from "./mask-api-key-DFWGlM7w.js";
import { a as formatElevatedRuntimeHint, c as formatInternalVerboseCurrentReplyOnlyText, i as formatDirectiveAck, l as formatInternalVerbosePersistenceDeniedText, n as canPersistInternalVerboseDirective, o as formatElevatedUnavailableText, r as enqueueModeSwitchEvents, s as formatInternalExecPersistenceDeniedText, t as canPersistInternalExecDirective, u as withOptions } from "./directive-handling.shared-ywkigXZM.js";
...
async function maybeHandleModelDirectiveInfo(params) {
	...
	for (const entry of pickerCatalog) {
		...
		const auth = await resolveAuthLabel(provider, params.cfg, modelsPath, params.agentDir, authMode, params.workspaceDir);
...
function maybeHandleQueueDirective(params) {
	...
	if (!directives.queueMode && !directives.queueReset && !directives.hasQueueOptions && directives.rawQueueMode === void 0 && directives.rawDebounce === void 0 && directives.rawCap === void 0 && directives.rawDrop === void 0) {
		...
		return { text: withOptions(`Current queue settings: mode=${settings.mode}, debounce=${debounceLabel}, cap=${capLabel}, drop=${dropLabel}.`, "modes steer, queue, followup, collect, steer+backlog, interrupt; debounce:<ms|s|m>, cap:<n>, drop:old|new|summarize") };
	}
	...
	if (queueModeInvalid || queueDebounceInvalid || queueCapInvalid || queueDropInvalid) {
		const errors = [];
		if (queueModeInvalid) errors.push(`Unrecognized queue mode "${directives.rawQueueMode ?? ""}". Valid modes: steer, queue, followup, collect, steer+backlog, interrupt.`);
...
async function handleDirectiveOnly(params) {
	...
	if (directives.hasThinkDirective && !directives.thinkLevel) {
		if (!directives.rawThinkLevel) return { text: withOptions(`Current thinking level: ${currentThinkLevel ?? "off"}.`, formatThinkingLevels(resolvedProvider, resolvedModel, ", ", thinkingCatalog)) };
		return { text: `Unrecognized thinking level "${directives.rawThinkLevel}". Valid levels: ${formatThinkingLevels(resolvedProvider, resolvedModel, ", ", thinkingCatalog)}.` };
	}
	...
	if (shouldPersistSessionEntry) {
		if (directives.hasThinkDirective && directives.thinkLevel && resolvedDirectiveThinkLevel) sessionEntry.thinkingLevel = resolvedDirectiveThinkLevel;
