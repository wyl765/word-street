...
import { l as listDeliverableMessageChannels } from "./message-channel-n3msLZX9.js";
import { n as buildMemoryPromptSection } from "./memory-state-Zcnt5VJy.js";
import { n as SILENT_REPLY_TOKEN } from "./tokens-B39_i7tu.js";
import { i as isKnownNativeApprovalPromptChannel, r as hasNativeApprovalPromptRuntimeCapability } from "./native-approval-prompt-DXpXre88.js";
import { n as normalizeStructuredPromptSection, t as normalizePromptCapabilityIds } from "./prompt-cache-stability-Cz37N143.js";
import { t as SYSTEM_PROMPT_CACHE_BOUNDARY } from "./system-prompt-cache-boundary-KiWNzJeq.js";
...
function buildMessagingSection(params) {
	...
	return [
		...
		hasSessionsSpawn ? hasSubagents ? "- Sub-agent orchestration → use `sessions_spawn(...)` to start delegated work; omit `context` for isolated children, set `context:\"fork\"` only when the child needs the current transcript; use `subagents(action=list|steer|kill)` to manage already-spawned children." : "- Sub-agent orchestration → use `sessions_spawn(...)` to start delegated work; omit `context` for isolated children, set `context:\"fork\"` only when the child needs the current transcript." : hasSubagents ? "- Sub-agent orchestration → use `subagents(action=list|steer|kill)` to manage already-spawned children." : "",
...
function buildDocsSection(params) {
	...
	return [
		...
		sourcePath ? `Local source: ${sourcePath}` : void 0,
		"Source: https://github.com/openclaw/openclaw",
		"Community: https://discord.com/invite/clawd",
		"Find new skills: https://clawhub.ai",
		docsPath ? "For OpenClaw behavior, commands, config, or architecture: consult local docs first." : "For OpenClaw behavior, commands, config, or architecture: consult the docs mirror first.",
		...
		sourcePath ? "If docs are incomplete or stale, inspect the local OpenClaw source code before answering." : "If docs are incomplete or stale, review the OpenClaw source on GitHub before answering.",
...
//#endregion
export { wrapUntrustedPromptDataBlock as i, buildAgentSystemPrompt as n, sanitizeForPromptLiteral as r, appendAgentBootstrapSystemPromptSupplement as t };
