...
import { a as coerceSecretRef, c as isValidEnvSecretRefId, p as resolveSecretInputRef } from "./types.secrets-BlhtUuXT.js";
...
import { n as discoverConfigSecretTargets, s as resolveConfigSecretTargetByPath } from "./target-registry-BuEgeeOk.js";
import { a as formatExecSecretRefIdValidationMessage, c as isValidSecretProviderAlias, d as validateExecSecretRefId, o as isValidExecSecretRefId, s as isValidFileSecretRefId, u as secretRefKey } from "./ref-contract-iNNZovFP.js";
import { n as info, o as success, t as danger } from "./globals-CZuktVBk.js";
import { a as resolveSecretRefValue } from "./resolve-B2bRy8Zo.js";
import { C as validateConfigObjectRaw, u as readConfigFileSnapshot, x as collectUnsupportedSecretRefPolicyIssues } from "./io-DDcMg_WY.js";
import { n as formatConfigIssueLines, o as normalizeConfigIssues } from "./issue-format-CEIVxsoT.js";
...
import { n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-C_gSriej.js";
import { n as setCommandJsonMode } from "./json-mode-CfauKv0L.js";
...
function unsetAtPath(root, path) {
	...
	for (let i = 0; i < path.length - 1; i += 1) {
		...
		if (Array.isArray(current)) {
			...
			const index = Number.parseInt(segment, 10);
			if (!Number.isFinite(index) || index < 0 || index >= current.length) return { removed: false };
			current = current[index];
			continue;
		}
		const record = current;
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/patch/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.openclaw.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-yCDESMkd.js");
