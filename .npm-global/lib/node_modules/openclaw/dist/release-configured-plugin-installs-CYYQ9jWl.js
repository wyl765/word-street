...
import { t as getOfficialExternalPluginCatalogEntry } from "./official-external-plugin-catalog--64MlR6o.js";
...
import { t as compareOpenClawVersions } from "./version-H9XZ5hLT.js";
import "./registry-ClLkIT5N.js";
import { a as listPotentialConfiguredChannelPresenceSignals, o as collectConfiguredAgentHarnessRuntimes } from "./config-presence-DsxCrJy0.js";
import { t as isChannelConfigured } from "./channel-configured-WWBqrmZ1.js";
import { r as detectPluginAutoEnableCandidates } from "./plugin-auto-enable-BUUTvE91.js";
...
import { n as repairMissingPluginInstallsForIds } from "./missing-configured-plugin-install-DRLHeUfu.js";
...
function collectSlotPluginIds(cfg) {
	...
	return ["memory", "contextEngine"].map((key) => normalizeId(slots?.[key])).filter((pluginId) => !!pluginId && pluginId.toLowerCase() !== "none");
}
...
function collectAgentHarnessRuntimePluginIds(cfg, env) {
	return collectConfiguredAgentHarnessRuntimes(cfg, env).map((runtime) => AGENT_HARNESS_RUNTIME_PLUGIN_IDS[runtime]).filter((pluginId) => Boolean(pluginId)).toSorted((left, right) => left.localeCompare(right));
}
