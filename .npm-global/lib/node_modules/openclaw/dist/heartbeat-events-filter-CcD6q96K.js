...
import { r as HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS } from "./heartbeat-B2uDcukR.js";
...
function buildCronEventPrompt(pendingEvents, opts) {
	...
	if (!eventText) {
		if (useHeartbeatResponseTool) return "A scheduled cron event was triggered, but no event content was found. " + HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS;
...
function buildExecEventPrompt(pendingEvents, opts) {
	...
	if (!eventText) {
		if (useHeartbeatResponseTool) return `An async command completion event was triggered, but no command output was found. ${HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS} Do not mention, summarize, or reuse output from any earlier run.`;
