import { n as normalizeDeviceMetadataForPolicy } from "./device-metadata-normalization-L3eRyFkL.js";
import { i as NODE_SYSTEM_RUN_COMMANDS, r as NODE_SYSTEM_NOTIFY_COMMAND, t as NODE_BROWSER_PROXY_COMMAND } from "./node-commands-w7VpFSbM.js";
...
function listDangerousPluginNodeCommands() {
	...
	if (!registry) return [];
	const commands = [...(registry.nodeHostCommands ?? []).filter((entry) => entry.command.dangerous === true).map((entry) => entry.command.command), ...(registry.nodeInvokePolicies ?? []).flatMap((entry) => entry.policy.commands)];
...
function resolveNodeCommandAllowlist(cfg, node) {
	...
	const allow = new Set([...base, ...extra].map((cmd) => cmd.trim()).filter((cmd) => cmd && !dangerousPluginCommands.has(cmd)));
...
//#endregion
export { resolveNodeCommandAllowlist as a, normalizeDeclaredNodeCommands as i, isNodeCommandAllowed as n, listDangerousPluginNodeCommands as r, DEFAULT_DANGEROUS_NODE_COMMANDS as t };
