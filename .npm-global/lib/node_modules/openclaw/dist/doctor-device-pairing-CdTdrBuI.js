...
import { t as JsonFileReadError } from "./json-files-DPM4MwsB.js";
...
import { c as listApprovedPairedDeviceRoles, g as summarizeDeviceTokens, l as listDevicePairing } from "./device-pairing-Czz_DnGP.js";
import fs from "node:fs";
import path from "node:path";
//#region src/commands/doctor-device-pairing.ts
function hasNumberVersion(value) {
	return "version" in value && typeof value.version === "number";
}
function isDeviceAuthStoreTokenEntry(value) {
	return typeof value === "object" && value !== null && "token" in value && typeof value.token === "string" && "role" in value && typeof value.role === "string" && "scopes" in value && Array.isArray(value.scopes) && value.scopes.every((scope) => typeof scope === "string") && "updatedAtMs" in value && typeof value.updatedAtMs === "number";
}
...
function collectPendingPairingIssues(snapshot) {
	...
	return snapshot.pending.map((pending) => formatPendingPairingIssue(resolvePendingPairingIssue(pending, pairedByDeviceId.get(pending.deviceId))));
}
function collectPairedRecordIssues(snapshot) {
	...
	for (const device of snapshot.paired) {
		...
		if (approvedRoles.includes("operator") && approvedScopes.length === 0) lines.push(`- Paired device ${deviceLabel} is missing its approved operator scope baseline. Scope upgrades can get stuck in pairing-required until the device repairs or is re-approved.`);
		for (const role of approvedRoles) {
			...
			if (!token) {
				lines.push(`- Paired device ${deviceLabel} has no active ${role} device token even though the role is approved. This commonly ends in pairing-required or device-token-mismatch. Rotate a fresh token with ${rotateCommand}.`);
				continue;
			}
			...
			})) lines.push(`- Paired device ${deviceLabel} has a ${role} token outside the approved scope baseline [${formatScopes(approvedScopes)}]. Rotate it with ${rotateCommand}.`);
		}
	}
	return lines;
}
...
function readLocalIdentity(env = process.env) {
	const identity = readJsonFile(path.join(resolveStateDir(env), "identity", "device.json"));
	if (!identity || typeof identity !== "object" || !hasNumberVersion(identity) || identity.version !== 1 || !("deviceId" in identity) || typeof identity.deviceId !== "string" || !identity.deviceId.trim()) return null;
	return {
		version: 1,
		deviceId: identity.deviceId
	};
}
function readLocalDeviceAuthStore(env = process.env) {
	const store = readJsonFile(path.join(resolveStateDir(env), "identity", "device-auth.json"));
	if (!store || typeof store !== "object" || !hasNumberVersion(store) || store.version !== 1 || !("deviceId" in store) || typeof store.deviceId !== "string" || !store.deviceId.trim() || !("tokens" in store) || typeof store.tokens !== "object" || store.tokens === null) return null;
	const tokens = {};
	for (const [role, entry] of Object.entries(store.tokens)) {
		if (!isDeviceAuthStoreTokenEntry(entry)) return null;
		tokens[role] = entry;
	}
	return {
		version: 1,
...
function collectLocalDeviceAuthIssues(snapshot) {
	const identity = readLocalIdentity();
	const store = readLocalDeviceAuthStore();
	if (!identity || !store || store.deviceId !== identity.deviceId) return [];
	...
	for (const entry of Object.values(store.tokens)) {
		...
		if (!pairedToken) {
			...
			lines.push(`- Local cached ${role} device auth for ${deviceLabel} no longer has a matching active gateway token. Reconnect with shared gateway auth to refresh it, or rotate with ${rotateCommand}.`);
			continue;
		}
		const gatewayIssuedAtMs = pairedToken.rotatedAtMs ?? pairedToken.createdAtMs;
		if (entry.updatedAtMs < gatewayIssuedAtMs) {
			lines.push(`- Local cached ${role} device token for ${deviceLabel} predates the gateway rotation. This is a stale device-token pattern and can fail with device token mismatch. Reconnect with shared gateway auth to refresh it, or rotate again with ${rotateCommand}.`);
			continue;
		}
		const cachedScopes = normalizeDeviceAuthScopes(entry.scopes);
		const pairedScopes = normalizeDeviceAuthScopes(pairedToken.scopes);
		if (cachedScopes.join("\n") !== pairedScopes.join("\n")) lines.push(`- Local cached ${role} device scopes for ${deviceLabel} differ from the gateway record. Cached scopes [${formatScopes(cachedScopes)}], gateway scopes [${formatScopes(pairedScopes)}]. Reconnect with shared gateway auth to refresh it, or rotate with ${rotateCommand}.`);
	}
	return lines;
}
function formatPairingStoreReadIssue(error) {
...
//#endregion
export { noteDevicePairingHealth };
