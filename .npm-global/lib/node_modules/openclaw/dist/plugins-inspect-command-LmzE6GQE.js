...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-D2dKqaOH.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-BHiDheBC.js");
	...
	if (opts.all) {
		...
		const report = runtimeInspect ? tracePluginLifecyclePhase("runtime plugin registry load", () => buildPluginDiagnosticsReport({
			config: cfg,
			...loggerParams
		}), {
			command: "inspect",
			all: true
		}) : tracePluginLifecyclePhase("plugin registry snapshot", () => buildPluginSnapshotReport({
			config: cfg,
			...loggerParams
		}), {
			command: "inspect",
	...
	const snapshotReport = tracePluginLifecyclePhase("plugin registry snapshot", () => buildPluginSnapshotReport({
		config: cfg,
		...loggerParams
	}), { command: "inspect" });
