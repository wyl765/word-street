...
function createLazyBrowserTool(opts) {
	...
	return {
		...
		execute: async (toolCallId, args, signal, onUpdate) => {
			const { createBrowserTool } = await import("./extensions/browser/register.runtime.js");
			return await createBrowserTool(opts).execute(toolCallId, args, signal, onUpdate);
		}
	};
}
...
const browserPluginNodeHostCommands = [{
	...
	handle: async (paramsJSON) => {
		const { runBrowserProxyCommand } = await import("./extensions/browser/register.runtime.js");
		return await runBrowserProxyCommand(paramsJSON);
	}
}];
const browserSecurityAuditCollectors = [async (ctx) => {
	const { collectBrowserSecurityAuditFindings } = await import("./extensions/browser/register.runtime.js");
...
function registerBrowserPlugin(api) {
	...
	api.registerGatewayMethod("browser.request", async (opts) => {
		const { handleBrowserGatewayRequest } = await import("./extensions/browser/register.runtime.js");
