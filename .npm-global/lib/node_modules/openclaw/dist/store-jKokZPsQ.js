...
import { d as readLocalFileSafely$1, t as SafeOpenError } from "./fs-safe-B_RfWeue.js";
import { n as detectMime, r as extensionForMime } from "./mime-BNqgx5w7.js";
import { t as retainSafeHeadersForCrossOriginRedirect } from "./redirect-headers-B3rVENiD.js";
import { h as resolvePinnedHostname } from "./ssrf-CUQ1WjrX.js";
...
function resolveMediaScopedDir(subdir, caller) {
	...
	if (relative && (relative === ".." || relative.startsWith(`..${path.sep}`))) throw new Error(`${caller}: media subdir escapes media directory: ${JSON.stringify(subdir)}`);
	return dir;
}
		...
		resolvePinnedHostnameImpl(parsedUrl.hostname).then((pinned) => {
			const req = requestImpl(parsedUrl, {
				...
				if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400) {
					...
					resolve(downloadToFile(redirectUrl, dest, new URL(redirectUrl).origin === parsedUrl.origin ? headers : retainSafeHeadersForCrossOriginRedirect(headers), maxRedirects - 1, maxBytes));
					return;
				}
...
async function writeSavedMediaBuffer(params) {
	...
	await retryAfterRecreatingDir(params.dir, async () => {
		...
		try {
			await fs$1.writeFile(tempDest, params.buffer, { mode: MEDIA_FILE_MODE });
			const handle = await fs$1.open(tempDest, "r+");
			try {
				await syncSavedMediaHandle(handle);
			} finally {
				await handle.close();
			}
			await fs$1.rename(tempDest, dest);
		} catch (err) {
			await fs$1.rm(tempDest, { force: true }).catch(() => {});
			throw err;
		}
	});
	return dest;
}
async function syncSavedMediaHandle(handle) {
	try {
		await handle.sync();
	} catch (err) {
...
async function saveMediaBuffer(buffer, contentType, subdir = "inbound", maxBytes = MAX_BYTES, originalFilename) {
	...
	const id = buildSavedMediaId({
		baseId: uuid,
		ext: headerExt ?? extensionForMime(mime) ?? safeOriginalFilenameExtension(originalFilename) ?? "",
		originalFilename
	});
	await writeSavedMediaBuffer({
		dir,
