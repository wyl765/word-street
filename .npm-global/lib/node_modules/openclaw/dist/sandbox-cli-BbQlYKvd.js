import { d as normalizeStringifiedOptionalString, s as normalizeOptionalLowercaseString } from "./string-coerce-Bje8XVt9.js";
...
import { c as normalizeAgentId, l as normalizeMainKey, r as buildAgentMainSessionKey, u as resolveAgentIdFromSessionKey } from "./session-key-C0K0uhmG.js";
...
import { i as removeSandboxContainer, n as listSandboxContainers, r as removeSandboxBrowserContainer, t as listSandboxBrowsers } from "./sandbox-CuE-5NHh.js";
...
function resolveActiveChannel(params) {
	...
	const normalizedCandidate = normalizeOptionalLowercaseString((entry?.lastChannel ?? entry?.channel ?? entry?.lastProvider ?? entry?.provider ?? "").trim());
...
async function sandboxExplainCommand(opts, runtime) {
	...
	const allowTokens = (values) => (values ?? []).map((v) => normalizeStringifiedOptionalString(v) ?? "").filter(Boolean);
