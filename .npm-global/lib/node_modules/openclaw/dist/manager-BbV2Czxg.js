...
import { n as isAcpSessionKey, o as parseAgentSessionKey } from "./session-key-utils-8PXPWO4Z.js";
...
import { i as failTaskRunByRunId, l as startTaskRunByRunId, r as createRunningTaskRun, t as completeTaskRunByRunId } from "./detached-task-runtime-BA5uIhZH.js";
import { a as withAcpRuntimeErrorBoundary, i as toAcpRuntimeError, n as AcpRuntimeError, t as ACP_ERROR_CODES } from "./errors-N-1tSJ3j.js";
import { a as identityHasStableSessionId, c as resolveRuntimeHandleIdentifiersFromIdentity, i as identityEquals, l as resolveRuntimeResumeSessionId, n as createIdentityFromHandleEvent, o as isSessionIdentityPending, r as createIdentityFromStatus, s as mergeSessionIdentity, t as createIdentityFromEnsure, u as resolveSessionIdentityFromMeta } from "./session-identity-BAj5cUum.js";
import { i as requireAcpRuntimeBackend, n as getAcpRuntimeBackend } from "./registry-B8_fAYa1.js";
import { i as upsertAcpSessionMeta, n as readAcpSessionEntry, t as listAcpSessionEntries } from "./session-meta-CCNCpcoO.js";
...
function buildRuntimeConfigOptionPairs(options) {
	...
	if (normalized.thinking) pairs.set("thinking", normalized.thinking);
...
function inferRuntimeOptionPatchFromConfigOption(key, value) {
	...
	if (normalizedKey === "thinking" || normalizedKey === "thought_level" || normalizedKey === "reasoning_effort") return { thinking: validateRuntimeThinkingInput(validated.value) };
		...
		await this.withSessionActor(sessionKey, async () => {
			...
			for (let attempt = 0; attempt < 2; attempt += 1) {
				...
				try {
					...
					if (!(await this.awaitTurnWithTimeout({
						sessionKey,
						turnPromise,
	...
	async closeSession(input) {
		...
		return await this.withSessionActor(sessionKey, async () => {
			...
			if (shouldSkipRuntimeClose) {
				...
				if (input.allowBackendUnavailable && (acpError.code === "ACP_BACKEND_MISSING" || acpError.code === "ACP_BACKEND_UNAVAILABLE" || input.discardPersistentState && acpError.code === "ACP_SESSION_INIT_FAILED" || input.discardPersistentState && acpError.code === "ACP_BACKEND_UNSUPPORTED_CONTROL" || this.isRecoverableAcpxExitError(acpError.message))) {
					if (input.discardPersistentState) {
						const configuredBackend = (meta.backend || input.cfg.acp?.backend || "").trim();
						try {
...
//#endregion
export { validateRuntimeConfigOptionInput as a, validateRuntimeModelInput as c, parseRuntimeTimeoutSecondsInput as i, validateRuntimePermissionProfileInput as l, getAcpSessionManager as n, validateRuntimeCwdInput as o, AcpSessionManager as r, validateRuntimeModeInput as s, __testing as t, resolveAcpSessionResolutionError as u };
