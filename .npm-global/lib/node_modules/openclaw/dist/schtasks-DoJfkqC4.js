...
import { f as resolveGatewayServiceDescription, m as resolveGatewayWindowsTaskName, t as resolveGatewayStateDir } from "./paths-nw72TSPj.js";
...
import { a as renderCmdSetAssignment, i as parseCmdSetAssignment, n as quoteCmdScriptArg, r as assertNoCmdLineBreak, t as parseCmdScriptCommandLine } from "./cmd-argv-CVlDd7Oj.js";
import { a as readWindowsProcessArgsSync, i as readWindowsListeningPidsOnPortSync, n as findGatewayPidsOnPortSync } from "./restart-stale-pids-Cw9Ycdol.js";
import { i as writeFormattedLines, n as formatLine, t as parseKeyValueOutput } from "./runtime-parse-CKvmuPPt.js";
import { a as inspectPortUsage } from "./ports-BOS4jQKS.js";
...
async function shouldFallbackScheduledTaskLaunch(params) {
	...
	const hasLaunchEvidence = async () => {
		...
		if (port) {
			if ((await resolveScheduledTaskGatewayListenerPids(port)).length > 0) return true;
		}
		...
		if (processSnapshot.error || processSnapshot.status !== 0) return false;
		let parsedSnapshot;
		try {
		...
		return entries.some((entry) => {
			...
			if (!isGatewayArgv(argv, { allowGatewayBinary: true })) return false;
