...
import { c as buildFoundryAuthResult, f as listConfiguredFoundryProfileIds, g as resolveFoundryApi, h as resolveConfiguredModelNameHint, i as PROVIDER_ID } from "./shared-CIy1QbVT.js";
import { c as listSubscriptions, o as getLoggedInAccount, s as isAzCliInstalled } from "./cli-DrE38wzc.js";
import { a as promptApiKeyEndpointAndModel, c as selectFoundryDeployment, i as loginWithTenantFallback, l as selectFoundryResource, o as promptEndpointAndModelManually, r as listResourceDeployments, s as promptTenantId, u as testFoundryConnection } from "./onboard-58yETqtF.js";
...
const entraIdAuthMethod = {
	...
	run: async (ctx) => {
		...
		if (selectedSub) if (await ctx.prompter.confirm({
			...
			const selectedDeployment = await selectFoundryDeployment(ctx, selectedResource, resourceDeployments);
...
const apiKeyAuthMethod = {
	...
	run: async (ctx) => {
		...
		return buildFoundryAuthResult({
			...
			modelNameHint: selection.modelNameHint ?? existingMetadata?.modelName ?? existingMetadata?.modelId,
			api: selection.api,
			authMethod: "api-key",
			currentProviderProfileIds: listConfiguredFoundryProfileIds(ctx.config),
			currentPluginsAllow: ctx.config.plugins?.allow,
