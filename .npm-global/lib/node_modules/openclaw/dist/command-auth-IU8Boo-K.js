...
import { r as listLoadedChannelPlugins, t as getLoadedChannelPluginById } from "./registry-loaded-DxBLokTx.js";
...
function resolveCommandAuthorization(params) {
	...
	const isOwnerForCommands = !(enforceOwner || ownerAllowlistConfigured) ? true : ownerState.ownerAllowAll ? true : ownerAllowlistConfigured ? senderIsOwner : senderIsOwnerByScope || Boolean(matchedCommandOwner);
	const isAuthorizedSender = resolveCommandSenderAuthorization({
		commandAuthorized,
		nativeCommandAuthorized: commandAuthorized && ctx.CommandSource === "native" && !ownerAllowlistConfigured,
		isOwnerForCommands,
		senderCandidates,
