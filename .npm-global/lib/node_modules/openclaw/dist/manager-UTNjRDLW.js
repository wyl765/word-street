...
import { p as resolveUserPath, y as truncateUtf16Safe } from "./utils-D5swhEXt.js";
import { b as resolveAgentDir, x as resolveAgentWorkspaceDir } from "./agent-scope-B6RIBoEj.js";
import { t as createSubsystemLogger } from "./subsystem-CxWoQXRD.js";
import { c as isUsageCountedSessionTranscriptFileName, i as isSessionArchiveArtifactName } from "./artifacts-CWcY_c7b.js";
...
import { t as extractKeywords } from "./query-expansion-wKB1alAH.js";
import { r as classifyMemoryMultimodalPath } from "./multimodal-DB_aXdAo.js";
...
import { u as enforceEmbeddingMaxInputTokens } from "./memory-core-host-engine-embeddings-B5UV9z-d.js";
import { a as ensureDir, c as normalizeExtraMemoryPaths, d as runWithConcurrency, f as isFileMissingError, i as cosineSimilarity, l as parseEmbedding, m as hasNonTextEmbeddingParts, n as buildMultimodalChunkForIndexing, r as chunkMarkdown, s as listMemoryFiles, t as buildFileEntry, u as remapChunkLines } from "./internal-g5sy5JDb.js";
import { t as hashText } from "./hash-Bt6RUWLc.js";
import "./memory-core-host-engine-foundation-CaBXuAqd.js";
import { c as buildSessionEntry, l as listSessionFilesForAgent, p as sessionPathForFile } from "./engine-qmd-DGVgOyCy.js";
import "./memory-core-host-engine-qmd-IJ3QuCIp.js";
import { n as readMemoryFile } from "./read-file-D5IDCM3N.js";
import { a as ensureMemoryIndexSchema, i as loadSqliteVecExtension, n as configureMemorySqliteWalMaintenance, r as requireNodeSqlite, t as closeMemorySqliteWalMaintenance } from "./engine-storage-Cn2Kt4hC.js";
import "./memory-core-host-engine-storage-BtuWZRLb.js";
import "./dreaming-shared-BqpWekl-.js";
import { t as canAutoSelectLocal } from "./provider-adapters-CQssBLda.js";
...
function isRetryableMemoryEmbeddingError(message) {
	return /(rate[_ ]limit|too many requests|429|resource has been exhausted|5\d\d|cloudflare|tokens per day|fetch failed|other side closed|ECONNRESET|ECONNREFUSED|ETIMEDOUT|EPIPE|UND_ERR_|socket hang up|network error|read ECONN|timed out)/i.test(message);
}
...
const transientRenameErrorCodes = new Set([
	"EBUSY",
	"EPERM",
	"EACCES"
]);
...
function isTransientRenameError(err) {
	return transientRenameErrorCodes.has(err.code ?? "");
}
async function renameWithRetry(source, target, options) {
	for (let attempt = 1; attempt <= options.maxRenameAttempts; attempt++) try {
		...
		if (!isTransientRenameError(err) || attempt === options.maxRenameAttempts) throw err;
...
async function moveMemoryIndexFiles(sourceBase, targetBase, options = {}) {
	...
	for (const suffix of [
		...
		"-wal",
		"-shm"
	]) await renameWithRetry(`${sourceBase}${suffix}`, `${targetBase}${suffix}`, resolvedOptions);
}
async function removeMemoryIndexFiles(basePath, fileOps = defaultFileOps) {
	await Promise.all([
		"",
		"-wal",
		"-shm"
	].map((suffix) => fileOps.rm(`${basePath}${suffix}`, { force: true })));
}
...
var MemoryManagerSyncOps = class {
	...
	async runSync(params) {
		...
		const targetedSessionSync = await runMemoryTargetedSessionSync({
			...
			useUnsafeReindex: process.env.OPENCLAW_TEST_FAST === "1" && process.env.OPENCLAW_TEST_MEMORY_UNSAFE_REINDEX === "1",
			sessionsDirtyFiles: this.sessionsDirtyFiles,
...
function resolveEmbeddingTimeoutMs(params) {
	if (params.kind === "query") {
		...
		if (typeof runtimeTimeoutMs === "number" && runtimeTimeoutMs > 0) return runtimeTimeoutMs;
	...
	if (typeof configuredTimeoutSeconds === "number" && configuredTimeoutSeconds > 0) return configuredTimeoutSeconds * 1e3;
...
function searchChunksByEmbedding(params) {
	...
	const rows = params.db.prepare(`SELECT id, path, start_line, end_line, text, embedding, source
  FROM chunks
 WHERE model = ?${params.sourceFilter.sql}`).iterate(params.providerModel, ...params.sourceFilter.params);
	const topResults = [];
	for (const row of rows) {
...
var MemoryIndexManager = class MemoryIndexManager extends MemoryManagerEmbeddingOps {
	...
	static async get(params) {
		...
		return await getOrCreateManagedCacheEntry({
			...
			pending: INDEX_CACHE_PENDING,
			key,
			bypassCache: purpose === "status" || purpose === "cli",
	...
	async search(query, opts) {
		...
		const keywordResults = hybrid.enabled && this.fts.enabled && this.fts.available ? await this.searchKeyword(cleaned, candidates, void 0, sourceFilterList).catch((err) => {
