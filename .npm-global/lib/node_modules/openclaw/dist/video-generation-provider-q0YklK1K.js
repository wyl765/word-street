...
async function pollRunwayTask(params) {
	...
	for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
		const response = await fetchWithTimeout(`${params.baseUrl}/v1/tasks/${params.taskId}`, {
			method: "GET",
		...
		switch (payload.status) {
			...
			case "FAILED":
			case "CANCELLED": throw new Error(normalizeOptionalString(typeof payload.failure === "string" ? payload.failure : payload.failure?.message) || `Runway video generation ${normalizeLowercaseStringOrEmpty(payload.status)}`);
			default:
	...
	return {
		...
		async generateVideo(req) {
			...
			try {
				...
				const outputUrls = completed.output?.map((value) => normalizeOptionalString(value)).filter((value) => Boolean(value));
