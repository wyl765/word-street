...
import { a as isSandboxHostPathAbsolute, c as splitSandboxBindSpec, t as getBlockedNetworkModeReason } from "./network-mode-DPBl_ITT.js";
import { C as SecretInputSchema, D as ToolsLinksSchema, O as ToolsMediaSchema, c as GroupChatSchema, d as HumanDelaySchema, f as IdentitySchema, j as TtsConfigSchema } from "./zod-schema.core-BebEss03.js";
...
const AgentContextLimitsSchema = z.object({
	memoryGetMaxChars: z.number().int().min(1).max(25e4).optional(),
	memoryGetDefaultLines: z.number().int().min(1).max(5e3).optional(),
	toolResultMaxChars: z.number().int().min(1).max(25e4).optional(),
	postCompactionMaxChars: z.number().int().min(1).max(5e4).optional()
}).strict().optional();
const AgentSkillsLimitsSchema = z.object({ maxSkillsPromptChars: z.number().int().min(0).optional() }).strict().optional();
...
const TrimmedOptionalConfigStringSchema = z.string().transform((value) => {
...
const ToolLoopPostCompactionGuardSchema = z.object({ windowSize: z.number().int().positive().optional() }).strict().optional();
...
const AgentEmbeddedHarnessSchema = z.object({ runtime: z.string().optional() }).strict().optional();
const AgentRuntimePolicySchema = z.object({ id: z.string().optional() }).strict().optional();
...
//#endregion
export { AgentSandboxSchema as a, MemorySearchSchema as c, AgentModelSchema as d, AgentRuntimePolicySchema as i, ToolPolicySchema as l, AgentEmbeddedHarnessSchema as n, ElevatedAllowFromSchema as o, AgentEntrySchema as r, HeartbeatSchema as s, AgentContextLimitsSchema as t, ToolsSchema as u };
