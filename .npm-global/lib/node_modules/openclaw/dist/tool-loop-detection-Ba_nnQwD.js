...
function resolveLoopDetectionConfig(config) {
	...
	return {
		...
		warningThreshold,
		unknownToolThreshold: asPositiveInt(config?.unknownToolThreshold, DEFAULT_LOOP_DETECTION_CONFIG.unknownToolThreshold),
		criticalThreshold,
...
function hashExecToolOutcome(details, text) {
	...
	if (!status) return;
	if (status === "running") return digestStable({
		status,
		tail: stringField(details.tail) ?? ""
	});
	if (status === "completed" || status === "failed") return digestStable({
		status,
		exitCode: typeof details.exitCode === "number" ? details.exitCode : null,
		timedOut: details.timedOut === true,
		output: nonEmptyStringField(details.aggregated) ?? text
	});
	if (status === "approval-pending" || status === "approval-unavailable") return digestStable({
		status,
...
function hashToolOutcome(toolName, params, result, error) {
	...
	if (!isPlainObject(result)) return { resultHash: result === void 0 ? void 0 : digestStable(result) };
	...
	return { resultHash: digestStable({
		details,
		text
	}) };
}
...
//#endregion
export { recordToolCall as n, recordToolCallOutcome as r, detectToolCallLoop as t };
