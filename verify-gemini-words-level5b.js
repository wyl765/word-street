...
const fileContent = fs.readFileSync('./words-level5b.js', 'utf8');
const match = fileContent.match(/const LEVEL5B_BANK\s*=\s*(\[[\s\S]*\]);/);

if (!match) {
  console.error('Could not parse words-level5b.js');
  process.exit(1);
}

const wordBank = eval(match[1]);

let output = '# Gemini Verification Report: words-level5b.js\n\n';
...
fs.writeFileSync('VERIFY-GEMINI-words-level5b-GATE.md', output);
