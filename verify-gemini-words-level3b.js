...
const match = fileContent.match(/const LEVEL3B_BANK\s*=\s*(\[[\s\S]*\]);/);

if (!match) {
  console.error('Could not parse words-level3b.js');
  process.exit(1);
}

const wordBank = eval(match[1]);

let output = '# Gemini Verification Report: words-level3b.js\n\n';
...
fs.writeFileSync('VERIFY-GEMINI-words-level3b-GATE.md', output);
