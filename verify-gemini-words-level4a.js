...
const fileContent = fs.readFileSync('./words-level4a.js', 'utf8');
const match = fileContent.match(/const LEVEL4A_BANK\s*=\s*(\[[\s\S]*\]);/);

if (!match) {
  console.error('Could not parse words-level4a.js');
  process.exit(1);
}

const wordBank = eval(match[1]);

let output = '# Gemini Verification Report: words-level4a.js\n\n';
...
fs.writeFileSync('VERIFY-GEMINI-words-level4a-GATE.md', output);
