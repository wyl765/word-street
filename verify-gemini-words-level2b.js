...
const fileContent = fs.readFileSync('./words-level2b.js', 'utf8');
const match = fileContent.match(/const LEVEL2B_BANK\s*=\s*(\[[\s\S]*\]);/);

if (!match) {
  console.error('Could not parse words-level2b.js');
  process.exit(1);
}

const wordBank = eval(match[1]);

let output = '# Gemini Verification Report: words-level2b.js\n\n';
...
fs.writeFileSync('VERIFY-GEMINI-words-level2b-GATE.md', output);
