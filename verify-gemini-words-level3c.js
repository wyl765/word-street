...
const fileContent = fs.readFileSync('./words-level3c.js', 'utf8');
const match = fileContent.match(/const LEVEL3C_BANK\s*=\s*(\[[\s\S]*\]);/);

if (!match) {
  console.error('Could not parse words-level3c.js');
  process.exit(1);
}

const wordBank = eval(match[1]);

let output = '# Gemini Verification Report: words-level3c.js\n\n';
...
fs.writeFileSync('VERIFY-GEMINI-words-level3c-GATE.md', output);
